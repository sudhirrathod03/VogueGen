...
function Products() {
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center bg-[#F8FAFC]">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-gray-200 border-t-[#185FA5]"></div>
        <p className="mt-4 text-sm font-medium text-gray-400 tracking-tight">Loading listed records...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 sm:p-8 font-sans antialiased text-gray-800">
      ...
      {/* Header Area */}
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 mb-6 gap-4 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-gray-900 tracking-tight sm:text-3xl">Products</h1>
        ...
        {token && (
          <Link
            to="/add-product"
            className="inline-flex items-center gap-2 px-4.5 py-2.5 bg-[#185FA5] hover:bg-[#144F8A] text-white text-xs font-semibold rounded-xl shadow-[0_4px_12px_rgba(24,95,165,0.15)] hover:shadow-[0_4px_16px_rgba(24,95,165,0.25)] transition-all duration-200 active:scale-95 whitespace-nowrap"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-3 items-center">
          ...
          <div className="relative w-full lg:flex-1">
            ...
            <input
              ...
              className="w-full bg-gray-50/50 border border-gray-100 pl-10 pr-4 py-2.5 rounded-xl focus:outline-none focus:ring-4 focus:ring-[#185FA5]/5 focus:border-[#185FA5] focus:bg-white text-sm text-gray-700 placeholder-gray-400/80 transition-all duration-200"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row w-full lg:w-auto items-center gap-3">
            ...
            <div className="relative w-full sm:w-56" ref={categoryDropdownRef}>
              <button
                ...
                className="w-full bg-white border border-gray-200/80 px-4 py-2.5 rounded-xl flex items-center justify-between text-sm text-gray-600 font-semibold hover:bg-gray-50 shadow-sm hover:border-gray-300 transition-all duration-200 cursor-pointer focus:outline-none focus:ring-4 focus:ring-[#185FA5]/5"
              >
                ...
                <ChevronDown className={`w-4 h-4 text-gray-400/80 transition-transform duration-200 flex-shrink-0 ${isCategoryOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              {isCategoryOpen && (
                <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-white border border-gray-100 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] max-h-60 overflow-y-auto py-1.5 animate-in fade-in slide-in-from-top-1 duration-100">
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 hover:bg-gray-50 transition-colors ${category === "" ? "bg-blue-50/50 text-[#185FA5] font-bold" : "text-gray-600 font-medium"}`}
                  >
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 hover:bg-gray-50 transition-colors ${category === cat ? "bg-blue-50/50 text-[#185FA5] font-bold" : "text-gray-600 font-medium"}`}
                    >
            ...
            <div className="relative w-full sm:w-48" ref={sortDropdownRef}>
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 text-gray-400/80 transition-transform duration-200 flex-shrink-0 ${isSortOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                <div className="absolute top-full right-0 left-0 z-50 mt-2 bg-white border border-gray-100 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] py-1.5 animate-in fade-in slide-in-from-top-1 duration-100">
                  {sortOptions.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors ${sort === option.value ? "bg-blue-50/50 text-[#185FA5] font-bold" : "text-gray-600 font-medium"}`}
                    >
                      {option.label}
                    </button>
                  ))}
          ...
          <Link to={`/products/${prod._id}`} key={prod._id} className="block group">
            <div className="bg-white rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.07)] transition-all duration-300 flex flex-col h-full transform hover:-translate-y-1">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <h2 className="text-base font-bold text-gray-900 mb-1.5 group-hover:text-[#185FA5] transition-colors duration-200 line-clamp-1 tracking-tight">
                  {prod.name}
                </h2>
      ...
      {product.length === 0 && (
        <div className="max-w-md mx-auto mt-16 p-8 text-center bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-100">
          ...
          <h3 className="text-sm font-bold text-gray-800 tracking-tight">No Inventory Records Found</h3>
          <p className="text-xs text-gray-400 mt-1 max-w-xs mx-auto leading-relaxed">Try broadening your active filters or tweaking your current keywords.</p>
        </div>
      )}
    </div>
  );
